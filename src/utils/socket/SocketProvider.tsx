...
const SocketProvider: FC<Props> = ({ children }) => {
    ...
    useEffect(() => {
        ...
        socket.connection.on("connect_error", (err) => {
 ...
 * 
 */
export function useListenEvent<Ev extends keyof ServerToClientEvents>(event: Ev, listener: ServerToClientEvents[Ev], deps?: DependencyList) {
    useEffect(() => {
